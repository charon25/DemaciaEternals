<?php
    ...
    if (!array_key_exists('time', $DATA)) {
        echo '{"status":{"message":"Bad Request - Data does not contain time","status_code":400}}';
        exit;
    }
    ...
    if (!is_array($row)) {
        ...
        $request = $bdd->prepare('UPDATE `et2_users` SET `time`=? WHERE `user_id`=?');
        $request->execute(array($DATA['time'], $_GET['user_id']));
        echo '{"status":{"message":"User time updated","status_code":200}}';
    }
